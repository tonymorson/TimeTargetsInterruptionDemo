...
public struct Report: Codable, Equatable {
  ...
  public init(workPattern: WorkPattern, dailyTarget: Int, tick: Tick) {
    let tick = max(0, tick)
  ...
  public init(periods: [(Period.Kind, Duration)], dailyTarget: Int, tick: Tick) {
    let tick = max(0, tick)
    ...
    self.init(periods: periods, dailyTarget: dailyTarget, tick: tick)
  }

  private init(periods: [Period], dailyTarget: Int, tick: Tick) {
    let tick = max(0, tick)
  ...
  public init(sessions: [[(Period.Kind, Duration)]], dailyTarget: Int, tick: Tick) {
    self.tick = tick
